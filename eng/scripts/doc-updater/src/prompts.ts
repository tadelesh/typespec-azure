...
export async function buildDocUpdatePrompt(
  ...
  prompt += `\n## Knowledge Base\n\n`;
  prompt += `As you work, record anything useful you discover into: \`${knowledgePath}\`\n\n`;
  prompt += `This includes: API signatures, feature mappings, doc conventions, cross-references, `;
  prompt += `gaps you find, patterns you notice — anything that would help you (or a future agent) `;
  prompt += `do this job better next time. Update the knowledge file incrementally as you go.\n\n`;
  prompt += `After updating the knowledge file, run \`pnpm format:dir ${knowledgePath}\` to format it.\n`;
