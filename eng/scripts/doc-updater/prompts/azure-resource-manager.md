...
3. **Use library templates and spread patterns instead of raw TypeSpec.** When a doc example writes raw TypeSpec (manual decorators, hand-rolled models) for something the library provides a template or spread pattern for, replace it with the idiomatic template usage. Always follow the patterns shown in `packages/samples/specs/resource-manager/` — those samples are the canonical examples of correct template usage. Only pass template parameters whose values differ from the template's defaults — do not duplicate a parameter that already matches its default value.
4. **Replace deprecated API names.** When code examples or text reference deprecated template names or type aliases, update them to the current names. Check the source for `#deprecated` annotations to identify what has been renamed.
5. **Preserve semantics in every replacement.** When replacing code with a template or spread pattern, carry over all values from the original. Simplifying syntax is fine; losing information is not.
...
- How-to guides: accurately reflect current library API and usage patterns, replace deprecated syntax with current syntax, fix incorrect information, and cover all critical scenarios service teams need to know to implement specific features or patterns in their ARM specs
...
Format every changed file by running `pnpm format` from the repository root.
