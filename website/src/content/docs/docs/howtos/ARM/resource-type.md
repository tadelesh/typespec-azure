...
`...ResourceNameParameter<EmployeeResource, KeyName = "employeeName", SegmentName = "employees">`: spreads in the standard resource name property. The `KeyName` parameter provides the parameter name for the name of the resource in operations that use this resource. The `SegmentName` parameter provides the resource type name for this resource.
...
`...ResourceNameParameter<JobResource, KeyName = "jobName", SegmentName = "jobs">`: spreads in the standard resource name property. The `KeyName` parameter provides the parameter name for the name of the resource in operations that use this resource. The `SegmentName` parameter provides the resource type name for this resource.
...
`@tenantResource`: designates this resource as being a cross-tenant resource, with scope across all customer subscriptions in the tenant.
`...ResourceNameParameter<EmployeeAgreementResource>`: provides the standard resource name property with auto-generated `@key`, `@segment`, and `@path` decorators.
...
More details on what to consider when using complex types in an array are discussed in the section on [modeling arrays of complex types](#modeling-arrays-of-complex-types).
...
Standard support for customer-managed key (CMK) encryption for a resource. To enable encryption support, add the `EncryptionProperty` envelope property to the resource definition:
...
Standard support for extended locations (Edge Zones, Custom Locations) for a resource. To enable extended location support, add the `ExtendedLocationProperty` envelope property to the resource definition:
...
Standard support for availability zones for a resource. To enable availability zone support, add the `AvailabilityZonesProperty` envelope property to the resource definition:
