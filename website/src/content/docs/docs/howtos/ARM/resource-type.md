...
`...ResourceNameParameter<EmployeeResource>`: spreads in the standard resource name parameter, which automatically applies the following decorators with values derived from the resource name:
- `@key(employeeName)`: provides the parameter name for this resource type in the service URI hierarchy.
- `@segment(employees)`: provides the resource type name for this resource, the URI segment that comes just before the parameter which identifies the resource.
...
- `@pattern`: provides a default regex pattern constraint for resource names.
...
`...ResourceNameParameter<EmployeeResource>`: spreads in the standard resource name parameter, automatically deriving the key name, segment, path, and pattern from the resource name.
...
`...ResourceNameParameter<JobResource>`: spreads in the standard resource name parameter, automatically deriving the key name, segment, path, and pattern from the resource name.
...
`@visibility(Lifecycle.Read)`: indicates that this property is returned in the body of responses to operations over this resource, but does not appear in the body of requests. Later sections describe the [usage of property visibility](#property-visibility-and-other-constraints).
`@path`: indicates that this property corresponds to the last segment of the url path to the resource (otherwise known as the resource identity).
...
- **ManagedBy**: A standard mechanism for indicating the resource is managed by another resource.
- **ExtendedLocation**: Support for extended locations such as custom locations.
- **Encryption**: Standard support for configuring encryption for a resource.
- **AvailabilityZones**: Standard support for availability zones.
...
Support for extended locations such as custom locations. To add extended location support, add the `ExtendedLocationProperty` envelope property to the resource definition:
...
Standard support for configuring encryption for a resource. To add encryption support, add the `EncryptionProperty` envelope property to the resource definition:
...
Standard support for availability zones. To add availability zone support, add the `AvailabilityZonesProperty` envelope property to the resource definition:
