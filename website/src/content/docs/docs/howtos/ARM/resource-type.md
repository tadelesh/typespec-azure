...
Support for extended locations for resources that can be deployed to Azure Edge Zones or other extended locations. To add extended location support, add the `ExtendedLocationProperty` envelope property to the resource definition:
...
Support for availability zones for resources that can be deployed across multiple zones for high availability. To add availability zones support, add the `AvailabilityZonesProperty` envelope property to the resource definition:
...
Support for customer-managed key (CMK) encryption configuration. To add encryption support, add the `EncryptionProperty` envelope property to the resource definition:
