...
- If a resource allows both generated (SystemAssigned) and user-assigned (UserAssigned) Managed Identity, use the spread (...) operator to include the `ManagedServiceIdentityProperty` standard envelope property. This will allow users to manage any ManagedServiceIdentity associated with this resource.
...
Support for extended location (Azure Stack HCI / edge). To add extended location to a resource, add the `ExtendedLocationProperty` standard envelope property to the resource definition:
...
Support for customer-managed key encryption. To add encryption configuration to a resource's properties, use the `EncryptionProperty` spread model:
...
Support for availability zones. To add availability zone support to a resource, use the `AvailabilityZonesProperty` standard envelope property:
