...
The `@nextLinkVerb` decorator specifies the HTTP verb (`"GET"` or `"POST"`) to use when following next-link pagination URLs. By default, next-link requests use `GET`. Use this decorator when the service requires `POST` for pagination.
...
```python
# Paging uses POST for next-link requests internally
class ItemServiceClient:
    def list_items(self, *, id: str, name: str, **kwargs: Any) -> ItemPaged[Item]: ...
```
...
public partial class ItemServiceClient
{
    public virtual Pageable<Item> GetItems(string id, string name, CancellationToken cancellationToken = default);
    public virtual AsyncPageable<Item> GetItemsAsync(string id, string name, CancellationToken cancellationToken = default);
}
```
