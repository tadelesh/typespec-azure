...
:::caution
The decorators in this section are in the `Azure.ClientGenerator.Core.Legacy` namespace and are considered legacy functionality. They should only be used when standard TypeSpec paging patterns are not feasible. All support an optional `scope` parameter for language-specific targeting.
:::

### `@markAsPageable`

Forces an operation to be treated as a pageable operation even when it does not follow standard paging patterns (e.g., not decorated with `@list`). When applied, SDK generators will generate paging mechanisms (iterators/async iterators) and return pageable-specific return types.

The decorator will automatically apply `@pageItems` to a `value` property on the response model if it is not already decorated.
...
Prevents an operation from being treated as a pageable operation even when it follows standard paging patterns (e.g., decorated with `@list`). The response will be the paged model itself rather than a list of items, and no paging mechanisms will be generated.
...
@get
op listItems(): ItemListResult;
// Returns ItemListResult directly, not an iterator of Item
```

### `@nextLinkVerb`

Specifies the HTTP verb for fetching the next page in a paging scenario. By default, the next link operation uses `GET`. Use this decorator when the next page must be fetched with `POST` instead.
