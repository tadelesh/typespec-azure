...
The `@scope` decorator restricts an operation or model property so that it is only generated for the specified language emitters.
This is useful when an operation should only be available in certain SDKs.

In the following example, the `create` operation is scoped to Python and Java only.
C#, TypeScript, and Go SDKs will not include the `create` method.
...
@@scope(WidgetService.Widgets.create, "python, java");
```

```python
class WidgetsOperations:
    # create is included because Python is in scope
    def list(self, **kwargs: Any) -> List[Widget]: ...
    def read(self, id: str, **kwargs: Any) -> Widget: ...
    def create(self, widget: Union[Widget, JSON, IO[bytes]], **kwargs: Any) -> Widget: ...
```

```csharp
// create is NOT included because C# is not in scope
...
```typescript
// TypeScript emitter may not filter by @scope — check emitter support
```

```java
public final class WidgetServiceClient {
    // create is included because Java is in scope
    public List<Widget> list() { ... }
    public Widget read(String id) { ... }
    public Widget create(Widget widget) { ... }
}
```

```go
// create is NOT included because Go is not in scope
type WidgetServiceWidgetsClient struct { }

func (client *WidgetServiceWidgetsClient) List(ctx context.Context, options *ListOptions) (ListResponse, error)
func (client *WidgetServiceWidgetsClient) Read(ctx context.Context, id string, options *ReadOptions) (ReadResponse, error)
// No Create method
```
...
The `@responseAsBool` decorator changes a HEAD operation's return type to `boolean`. When applied, a `2xx` response returns `true` and a `404` response returns `false`.
...
@route("/widgets")
interface Widgets {
  @head exists(@path id: string): OkResponse | NotFoundResponse;
  @get read(@path id: string): Widget;
}
```
...
@@responseAsBool(WidgetService.Widgets.exists);
```

```python
class WidgetsOperations:
    # exists returns bool: True for 2xx, False for 404
...
```typescript
// TypeScript returns a response object; boolean conversion depends on emitter implementation
```
...
```go
// TODO: fill in Go example manually — emitter generation failed (internal error)
```
