...
The `@responseAsBool` decorator tells emitters that a HEAD operation should return a boolean value. When applied, a `200 OK` response maps to `true`, a `404 Not Found` maps to `false`, and any other status code raises an error.
...
interface Widgets {
  @head
  @route("/{name}")
  checkExists(@path name: string): OkResponse | NotFoundResponse;
}
```
...
@@responseAsBool(WidgetService.Widgets.checkExists);
```

```python
class WidgetServiceClient:
    def check_exists(self, name: str, **kwargs: Any) -> bool:
        ...
```
...
{
    public virtual Response<bool> CheckExists(string name, string accept, CancellationToken cancellationToken = default)
    {
        ...
    }

    public virtual async Task<Response<bool>> CheckExistsAsync(string name, string accept, CancellationToken cancellationToken = default)
    {
        ...
    }
}
...
```typescript
export type WidgetsCheckExistsResponse = { body: boolean };

export interface WidgetsOperations {
  checkExists: (
    name: string,
    options?: WidgetsCheckExistsOptionalParams,
  ) => Promise<WidgetsCheckExistsResponse>;
}
```

```java
// TODO: fill in Java example manually — emitter returns BinaryData instead of boolean
```
...
The `@scope` decorator restricts an operation or model property to a specific set of language emitters. Operations decorated with `@scope` will only appear in the SDKs for the specified languages.
...
// The diagnostics operation will only be generated for Python
@@scope(PetStore.InternalOps.diagnostics, "python");
```

```python
# Python includes the diagnostics operation
class PetStoreClient:
    pets: PetsOperations
    internal_ops: InternalOpsOperations  # present only in Python

class InternalOpsOperations:
    def diagnostics(self, **kwargs: Any) -> str:
        ...
```

```csharp
// C# does NOT include the diagnostics operation
public partial class PetStoreClient
{
    public virtual Pets GetPetsClient();
    // No InternalOps client
}
```

```typescript
// TypeScript does NOT include the diagnostics operation
export class PetStoreClient {
  public readonly pets: PetsOperations;
  // No internalOps property
}
```

```java
// Java does NOT include the diagnostics operation
public final class PetStoreClient {
    public List<Pet> list();
    public Pet create(Pet pet);
    // No diagnostics method
}
```

```go
// Go does NOT include the diagnostics operation
type PetStoreClient struct {}
func (client *PetStoreClient) NewPetStorePetsClient() *PetStorePetsClient
// No InternalOps client
```
