...
The `@responseAsBool` decorator indicates that a HEAD operation should return a boolean instead of the standard response type. When applied:

- A `2xx` response returns `true`
- A `404` response returns `false` (instead of raising an error)
- All other error status codes still raise an error
...
@head
@responseAsBool
op checkResourceExists(@path name: string): OkResponse;
```

```python
# The method returns bool instead of a response object
# 2xx -> True, 404 -> False
...
```java
// Protocol method
public Response<BinaryData> checkResourceExistsWithResponse(
