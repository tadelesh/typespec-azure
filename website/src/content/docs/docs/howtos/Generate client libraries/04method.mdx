...
The `@scope` decorator controls which language emitters generate a given operation or model property. By default, all operations are generated for all languages. Use `@scope` to include an operation for specific languages only, or to exclude it from certain languages.
...
@route("/pets/{name}")
@delete
op deletePet(@path name: string): void;
```
...
// The `deletePet` operation will only be generated for Python and Java clients
@@scope(PetStore.deletePet, "python, java");
```

```python
class PetStoreClient:
    # `read` and `list` are generated for all languages
    def read(self, name: str, **kwargs: Any) -> Pet: ...
    def list(self, **kwargs: Any) -> List[Pet]: ...

    # `delete_pet` is generated because Python is in scope
    def delete_pet(self, name: str, **kwargs: Any) -> None: ...
```

```csharp
public partial class PetStoreClient
{
    // `Read` and `GetAll` are generated for all languages
    ...
    // `DeletePet` is NOT generated — C# is not in scope
}
```

```typescript
// TypeScript does not currently filter operations based on @scope
export class PetStoreClient {
  read(name: string, options?: ReadOptionalParams): Promise<Pet>;
  list(options?: ListOptionalParams): Promise<ListResponse>;
  deletePet(name: string, options?: DeletePetOptionalParams): Promise<void>;
}
```
...
public final class PetStoreClient {
    ...
    // `deletePet` is generated because Java is in scope
...
// `DeletePet` is NOT generated — Go is not in scope
```

</ClientTabs>

## Modeling HEAD operations as boolean responses

The `@responseAsBool` decorator transforms a HEAD operation so that it returns a boolean instead of `void`:

- **2xx** status codes return `true`
- **404** returns `false` (no error is raised)
- Other status codes still raise an error

This is useful for "exists" checks.
...
```python
class PetStoreClient:
    # Returns True if the pet exists (2xx), False if not found (404)
...
```typescript
export class PetStoreClient {
  exists(name: string, options?: ExistsOptionalParams): Promise<ExistsResponse>;
  read(name: string, options?: ReadOptionalParams): Promise<Pet>;
}

// ExistsResponse wraps the boolean result
type ExistsResponse = { body: boolean };
```

```java
public final class PetStoreClient {
    public BinaryData exists(String name);
    public Pet read(String name);
}
```
