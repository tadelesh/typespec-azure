...
The `@clientApiVersions` decorator allows you to add API versions that the client should support beyond those defined in the `@versioned` enum. This is useful when a client needs to support older API versions that existed before the TypeSpec definition, or when a client needs to support additional versions not present in the service definition.
...
enum ClientApiVersions {
  v1: "v1",
  ...WidgetService.Versions,
}

@@clientApiVersions(WidgetService, ClientApiVersions);
```

```python
# The client now supports v1, v2, and v3 (v1 was added via @clientApiVersions)
class WidgetServiceClient:
    def __init__(self, endpoint: str, *, api_version: str = "v3", **kwargs: Any): ...
...
```java
// The ServiceVersion enum includes only the original service versions
public enum WidgetServiceServiceVersion implements ServiceVersion {
    V2("v2"),
    V3("v3");

    public static WidgetServiceServiceVersion getLatest(); // V3
}
```

```go
// Generated from API version v3
func NewWidgetServiceClientWithNoCredential(endpoint string, options *WidgetServiceClientOptions) (*WidgetServiceClient, error)
```
