...
By default, the client's API version enum only includes versions defined by the `@versioned` decorator on the service namespace. The `@clientApiVersions` decorator lets you extend this enum with additional versions — for example, older versions that existed before versioning decorators were added to the spec.

This is particularly useful when generating a complete API version enum without requiring the entire specification to be annotated with versioning decorators.
...
enum AllApiVersions {
  ...
  ...DemoService.Versions,
}

@@clientApiVersions(DemoService, AllApiVersions);
```

```python
# Python: The client accepts api_version as a keyword argument.
# The extended versions are available for selection.
from demoservice import DemoServiceClient

client = DemoServiceClient(endpoint=..., credential=...)
# Default is latest: "2024-07-01"

client_with_old_version = DemoServiceClient(endpoint=..., credential=..., api_version="2023-01-01")
# Can now specify older versions from the extended enum
```

```csharp
// C# generates a ServiceVersion enum with all four versions
public enum ServiceVersion
{
    V2023_01_01 = 1,
    V2023_07_01 = 2,
...
DemoServiceClient client = new DemoServiceClient(endpoint);
// Override with an older version
DemoServiceClientOptions options = new DemoServiceClientOptions(ServiceVersion.V2023_01_01);
DemoServiceClient clientOld = new DemoServiceClient(endpoint, options);
```
...
```java
// Java generates a ServiceVersion enum
// Note: Java may only include versions from @versioned, not the extended enum
public enum DemoServiceVersion implements ServiceVersion {
    V2024_01_01("2024-01-01"),
    V2024_07_01("2024-07-01");

    public static DemoServiceVersion getLatest() { return V2024_07_01; }
}
```
