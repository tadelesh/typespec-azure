...
## Adding Client-Specific API Versions with `@clientApiVersions`

The `@clientApiVersions` decorator allows you to define additional API versions that the client can support beyond those defined in the service's versioning configuration. This is useful when you need to generate a complete API version enum without requiring the entire specification to be annotated with versioning decorators.
...
// Add older API versions alongside the service's versions
...
# Client uses the latest version by default
client = ServiceClient(endpoint=..., credential=...)

# Override to an older version added via @clientApiVersions
client = ServiceClient(endpoint=..., credential=..., api_version="2023-06-01")
```

```csharp
// The generated ServiceVersion enum includes all four versions
public enum ServiceVersion
{
    V2023_06_01 = 1,
    V2023_10_01 = 2,
