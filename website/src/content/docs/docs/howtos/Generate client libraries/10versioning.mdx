...
The `@clientApiVersions` decorator lets you specify additional API versions that the client can support beyond those defined in the service's `@versioned` configuration. This is useful when you want the client to expose older API versions that existed before the spec was annotated with `@versioned`, without requiring the entire specification to include versioning annotations.
...
// Extend the client to also expose older API versions
...
```python
# Python generates the client with the default (latest) API version.
# The extended API versions are available for the client to target.
from my.service import ServiceClient

client = ServiceClient(endpoint=...)
# client defaults to "2024-07-01"
```

```csharp
// The ServiceVersion enum includes all versions — both the extended
// older versions and the service-defined versions
public enum ServiceVersion
{
    V2023_01_01 = 1,
    V2023_07_01 = 2,
...
```java
// Java only exposes the service-defined versions
public enum ServiceServiceVersion implements ServiceVersion {
    V2024_01_01("2024-01-01"),
    V2024_07_01("2024-07-01");
