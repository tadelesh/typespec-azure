...
## Defining additional API versions with `@clientApiVersions`

The `@clientApiVersions` decorator allows you to specify additional API versions that a client supports beyond those defined by the service's `@versioned` decorator. This is useful when you want to generate a complete API version enum without requiring the entire specification to be annotated with versioning decorators.
...
With this configuration, the generated client will expose API versions `v1` through `v5`, even though the service spec only defines `v4` and `v5`. The default version will still be the last entry in the combined enum.
