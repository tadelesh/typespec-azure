...
You can also use `@apiVersion(false)` to explicitly mark a parameter as **not** an API version parameter, even if its name matches the default detection pattern (e.g., `api-version`):
...
In this case, the `api-version` parameter will remain as a regular operation parameter and will not be elevated to the client level.

## Extending Client API Versions with `@clientApiVersions`

The `@clientApiVersions` decorator lets you specify additional API versions that the client supports beyond those defined in the service's `@versioned` configuration. This is particularly useful when:

- You want to expose a complete API version enum without annotating the entire spec with versioning decorators
- The client needs to support versions that aren't in the service specification
...
```python
# The client will expose all versions: v1, v2, v3, v4, v5
from my.service import MyServiceClient

client = MyServiceClient(endpoint=..., credential=..., api_version="v3")
```

```csharp
// ServiceVersion enum includes all versions
public enum ServiceVersion
{
    V1 = 1,
    V2 = 2,
...
public enum ServiceServiceVersion implements ServiceVersion {
    ...
    public static ServiceServiceVersion getLatest() {} // V5
}
```
