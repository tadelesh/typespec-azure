...
## Extending API Versions with `@clientApiVersions`

By default, the generated client only exposes the API versions defined in the service's `@versioned` enum. If you need to extend the API version list — for example, to include older versions that existed before the spec was annotated with versioning decorators — you can use the `@clientApiVersions` decorator.

This decorator takes an enum that includes both the additional versions and the service's existing versions (via spread), and applies it to the service namespace.
...
@route("/users")
@get
op getUser(@query name: string): User;
```
...
// Include older versions not in the service's @versioned enum
...
# The client now supports all four API versions.
# The default is still the latest: "2024-10-01"
client = MyServiceClient(endpoint=...)

# You can target an older version that was added via @clientApiVersions
older_client = MyServiceClient(endpoint=..., api_version="2023-06-01")
```
...
ServiceClient client = new ServiceClient(endpoint);
// client's version defaults to "2024-10-01"

ServiceClientOptions options = new ServiceClientOptions(ServiceClientOptions.ServiceVersion.V2023_06_01);
ServiceClient olderClient = new ServiceClient(endpoint, options);
// client's version is now "2023-06-01"
```

```typescript
/** Known values of ClientApiVersions that the service accepts. */
...
public enum ServiceServiceVersion implements ServiceVersion {
    ...
    public static ServiceServiceVersion getLatest() {} // V2024_10_01
}
```

```go
// Go does not expose an API version enum directly.
// The client sends the version configured at initialization.
```
