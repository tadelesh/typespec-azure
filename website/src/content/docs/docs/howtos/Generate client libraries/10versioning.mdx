...
## Extending Client API Versions

The `@clientApiVersions` decorator allows you to extend the set of API versions exposed by the client beyond what the service's `@versioned` decorator defines. This is useful when you want to generate a complete API version enum without annotating the entire specification with versioning decorators.
...
// Define additional API versions that the client should support
enum ClientVersions {
...
@@clientApiVersions(Demo, ClientVersions);
```

```python
# The client exposes all 4 API versions (2 additional + 2 from service)
from demo import DemoClient

client = DemoClient(endpoint=..., credential=...)
# Default version is the latest: "2024-06-01"
```

```csharp
// The ServiceVersion enum includes all 4 versions
public enum ServiceVersion
{
    V2023_01_01 = 1,
    V2023_06_01 = 2,
...
public partial class DemoClientOptions : ClientOptions
{
    public DemoClientOptions(ServiceVersion version = ServiceVersion.V2024_06_01);
}
```
...
public enum DemoServiceVersion implements ServiceVersion {
    ...
    public static DemoServiceVersion getLatest() { ... } // V2024_06_01
}
```
