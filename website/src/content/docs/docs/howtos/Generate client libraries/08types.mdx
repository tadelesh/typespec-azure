...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar type derived from `string`) should be deserialized as `null` when its value is an empty string (`""`). This is useful when a service returns empty strings to represent absent values.

This decorator can only be applied to properties whose type is `string` or a scalar derived from `string`.
...
@route("/users")
@post
op createUser(@body user: UserProfile): UserProfile;
```

```python
class UserProfile:
    name: str  # Required
    nickname: Optional[str]  # "" from the service is deserialized as None
    bio: Optional[str]  # "" from the service is deserialized as None
```
...
public partial class UserProfile
{
    ...
    public string Nickname { get; set; }  // "" from the service is deserialized as null
    public string Bio { get; set; }  // "" from the service is deserialized as null
}
```
...
export interface UserProfile {
  name: string;
  nickname?: string; // "" from the service is deserialized as null
  bio?: string; // "" from the service is deserialized as null
}
```
...
public final class UserProfile {
    ...
    private String nickname; // "" from the service is deserialized as null
    private String bio; // "" from the service is deserialized as null
}
```
...
type UserProfile struct {
	...
	Nickname *string // "" from the service is deserialized as nil
	Bio      *string // "" from the service is deserialized as nil
}
```
