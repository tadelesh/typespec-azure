...
:::caution
`@useSystemTextJsonConverter` is only used for backward compatibility with the C# emitter. You should not use it unless directed by SDK architects.
:::

The `@useSystemTextJsonConverter` decorator marks a model as needing a custom `System.Text.Json` converter in the generated C# code. This is a C#-specific decorator and has no effect on other language emitters.
...
The decorator accepts an optional `scope` parameter to restrict it to specific emitters:
