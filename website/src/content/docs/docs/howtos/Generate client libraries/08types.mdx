...
The `@deserializeEmptyStringAsNull` decorator indicates that a string-type model property should be treated as `null` when the service returns an empty string (`""`). This is useful for services that use empty strings to represent missing or unset values.

The decorator can only be applied to model properties whose type is `string` or a scalar derived from `string`.
...
@route("/users/{id}")
@get
op getUser(@path id: string): UserProfile;
```
...
class UserProfile(_Model):
    id: str = rest_field()
    name: str = rest_field()
    # Empty string "" will be deserialized as None
    nickname: Optional[str] = rest_field()
    bio: Optional[str] = rest_field()
```
...
public partial class UserProfile
{
    ...
    // Empty string "" will be deserialized as null
...
export interface UserProfile {
  id: string;
  name: string;
  // Empty string "" will be deserialized as undefined
  nickname?: string;
  bio?: string;
}
```
...
type UserProfile struct {
    ID       *string
    Name     *string
    // Empty string "" will be deserialized as nil
    Nickname *string
    Bio      *string
}
```
