...
The `@deserializeEmptyStringAsNull` decorator marks string properties so that empty strings (`""`) are deserialized as `null` values. This is useful for URL-typed or optional string properties where an empty string should be treated as absent.
...
model Pet {
  ...
  /** The URL of the pet's profile image. */
  profileImageUrl: url;
}
...
@@deserializeEmptyStringAsNull(PetStore.Pet.profileImageUrl);
```
...
class Pet(Model):
    id: str
    name: str
    # Empty string values for profile_image_url will be deserialized as None
...
{
    // Empty string values for ProfileImageUrl will be deserialized as null
    public Uri ProfileImageUrl { get; }
}
```

```typescript
// The decorator is consumed by emitters at deserialization time.
// The model shape is unchanged:
