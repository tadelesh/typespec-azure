...
The `@deserializeEmptyStringAsNull` decorator instructs emitters to deserialize an empty string (`""`) as `null` for the annotated property. This is useful for properties that may receive empty strings from the service when the value is semantically absent.
...
model Widget {
  ...
  @doc("The callback URL. An empty string means no callback is configured.")
  callbackUrl?: url;
}

@route("/widgets")
interface Widgets {
  @get read(@path name: string): Widget;
}
```
...
@@deserializeEmptyStringAsNull(WidgetService.Widget.callbackUrl);
```

```python
class Widget(_Model):
    callback_url: Optional[str] = rest_field(name="callbackUrl")
    """The callback URL. An empty string means no callback is configured."""

    def __init__(self, *, name: str, callback_url: Optional[str] = None) -> None:
        ...
```
...
public partial class Widget
{
    ...
    /// <summary> The callback URL. An empty string means no callback is configured. </summary>
