...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar derived from `string`) should be deserialized as `null` when its value is an empty string (`""`).

This is useful for services that return empty strings to represent missing or unset values.
...
:::note
The decorator can only be applied to properties whose type is `string` or a scalar type derived from `string`. Applying it to other types will result in a diagnostic error.
:::

## HEAD Operations as Boolean Response

The `@responseAsBool` decorator changes how a HEAD operation is modeled in the generated client SDK. Instead of returning the raw HTTP response, the operation returns a boolean:

- `true` for 2xx responses (resource exists)
- `false` for 404 responses (resource does not exist)
- Other status codes still raise errors

This decorator can only be applied to operations that use the `@head` HTTP verb.
...
@route("/resources/{name}")
@head
op resourceExists(@path name: string): void;
```

```python
class DemoServiceClient:
    def get_resource(self, name: str, **kwargs: Any) -> Resource: ...
    # Returns bool instead of raw response
    def resource_exists(self, name: str, **kwargs: Any) -> bool: ...
```

```csharp
public partial class DemoServiceClient
{
    public virtual Response<Resource> GetResource(string name, CancellationToken cancellationToken = default);
    // Returns Response<bool> instead of raw Response
    public virtual Response<bool> ResourceExists(string name, string accept, CancellationToken cancellationToken = default);
    public virtual async Task<Response<bool>> ResourceExistsAsync(string name, string accept, CancellationToken cancellationToken = default);
}
```

```typescript
export type ResourceExistsResponse = { body: boolean };

export async function resourceExists(
  context: Client,
  name: string,
  options?: ResourceExistsOptionalParams,
): Promise<ResourceExistsResponse>;
```

```java
public final class DemoServiceClient {
    public Resource getResource(String name);
    public BinaryData resourceExists(String name, String accept);
}
```
