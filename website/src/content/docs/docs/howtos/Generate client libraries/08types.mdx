...
## Using Custom JSON Converters (C# Only)

The `@useSystemTextJsonConverter` decorator indicates that a model should use a custom JSON converter for serialization and deserialization. This decorator is C#-specific and primarily used for backward compatibility with existing SDKs.

:::caution
This decorator is considered legacy functionality and should only be used when maintaining backward compatibility with existing C# SDKs. New services should rely on standard TypeSpec patterns for serialization.
:::
...
```python
# This decorator does not affect Python code generation
```
...
```csharp
// C# will generate a custom System.Text.Json converter for this model
...
```typescript
// This decorator does not affect TypeScript code generation
```
...
```java
// This decorator does not affect Java code generation
```
...
```go
// This decorator does not affect Go code generation
```
