...
In this example, instead of generating `MyModel` in the Python SDK, the emitter will reference `ExternalModel` from `externallib` (version 1.0.0 or later) instead.

The `ExternalType` model has the following properties:

- **`identity`** (required): The identity of the external type, e.g., `"pystac.Collection"`.
...
:::note
External types (with `identity` property) cannot be applied to model properties. They must be applied to the type definition itself (Scalar, Model, Enum, or Union). To change a model property's type, use a TypeSpec type directly instead of an `ExternalType`.
:::
...
- When the source type is `Scalar`, the alternate type must also be `Scalar`.
...
The `@deserializeEmptyStringAsNull` decorator indicates that a `string` (or `string`-derived scalar) property should be deserialized as `null` when its value is an empty string (`""`). This is useful for services that return empty strings where `null` is the intended semantic value.
...
class ResourceProperties:
    # When the service returns description: "", the SDK will set this to None
    description: Optional[str] = rest_field()
    # When the service returns external_id: "", the SDK will set this to None
...
{
    // When the service returns description: "", the SDK will set this to null
    public string Description { get; set; }
    // When the service returns externalId: "", the SDK will set this to null
...
type ResourceProperties struct {
    // When the service returns Description: "", the SDK will set this to nil
    Description *string
    // When the service returns ExternalID: "", the SDK will set this to nil
    ExternalID *string
    Name *string
}
```
