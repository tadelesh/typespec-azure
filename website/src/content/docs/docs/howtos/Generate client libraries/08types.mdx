...
The `@useSystemTextJsonConverter` decorator is a C#-specific decorator that indicates a model needs a custom `System.Text.Json` JSON converter. This is used only for backward compatibility in the C# emitter.

:::caution
This decorator is only relevant for C# code generation. Other language emitters will ignore it.
:::
...
```csharp
// The C# emitter generates a custom System.Text.Json converter for MyModel
