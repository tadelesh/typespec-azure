...
:::caution
Flattening is NOT a recommended pattern, and you shouldn't use it unless told by SDK architects. The `@flattenProperty` decorator is a legacy feature under the `Azure.ClientGenerator.Core.Legacy` namespace.
:::
...
The `@deserializeEmptyStringAsNull` decorator indicates that empty string values (`""`) should be deserialized as `null` in the generated client. This is useful when a service returns empty strings to represent absent values.
...
```python
# When the service returns {"callbackUrl": "", "description": ""},
# the deserialized values will be None instead of empty strings
```
...
```csharp
// When the service returns {"callbackUrl": "", "description": ""},
// the deserialized values will be null instead of empty strings
```
...
- Can only be applied to model properties, not to models or other targets.
- Only valid on `string` or `string`-derived scalar properties (e.g., `url`, custom scalars extending `string`).
- Applying it to non-string types (e.g., `int32`, `string[]`) will emit a diagnostic error.
- Supports language scoping: `@deserializeEmptyStringAsNull("csharp")` to apply only for C#.
