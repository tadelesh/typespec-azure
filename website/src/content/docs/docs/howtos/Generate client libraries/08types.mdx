...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar derived from `string`) should be deserialized as `null` when the service returns an empty string (`""`). This is useful for services that use empty strings to represent null values.

This decorator can only be applied to model properties whose type is `string` or a scalar type derived from `string`.
...
class Resource(_Model):
    ...
    # When the service returns description: "", it will be deserialized as None
```
...
{
    public string Name { get; }
    // When the service returns description: "", it will be deserialized as null
...
export interface Resource {
  name: string;
  description?: string;
  // When the service returns description: "", it will be deserialized as null/undefined
}
```
...
type Resource struct {
	Name        *string
	Description *string
	// When the service returns description: "", it will be deserialized as nil
}
```
