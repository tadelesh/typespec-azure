...
The `@alternateType` decorator allows you to override the emitted type for a model, union, scalar, enum, or union in generated client libraries. It can reference TypeSpec-defined types or external types from other packages.

### Example: Changing a Property Type

You can override a model property type inline or from `client.tsp`:
...
To reference a type from an external package, use an `ExternalType` object with the `identity` field (fully-qualified type name), an optional `package` name, and an optional `minVersion`:
...
In this example, instead of generating `ItemCollection` in the Python SDK, the emitter will reference `pystac.Collection` from the `pystac` package.

:::note
External types cannot be applied to model properties — apply them to the type definition instead. Applying an external type to a model property will emit a warning.
:::
...
- The `ExternalType` model has three fields: `identity` (required, fully-qualified name), `package` (optional), and `minVersion` (optional).
- When the source type is a `Scalar`, the alternate type must also be a `Scalar`.
- `@encode` from the source type is overridden by the alternate type's encoding.

## Deserializing empty strings as null

The `@deserializeEmptyStringAsNull` decorator marks a `string` (or `string`-based scalar) model property so that an empty string value (`""`) returned from the service is deserialized as `null` in the client. This is useful for brownfield services that use empty strings to signal absent values rather than `null`.
...
model Pet {
  ...
  /** An empty string from the service is treated as null */
...
class Pet(_model_base.Model):
    name: str = rest_field()
    """Required."""
    nickname: str = rest_field()
    """An empty string from the service is treated as null. Required."""

# Usage
pet = client.get_pet(name="fluffy")
# pet.nickname will be None if the service returned ""
```
...
namespace PetStore.Models
{
    public partial class Pet
    {
        ...
        // Empty string from service is deserialized as null at runtime
...
type Pet struct {
    Name     string
    // Empty string from service is deserialized as nil at runtime
    Nickname *string
}
```
