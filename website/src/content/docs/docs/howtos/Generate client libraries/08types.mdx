...
The `@deserializeEmptyStringAsNull` decorator marks string properties so that empty strings (`""`) are deserialized as `null` values. This is useful for URL-typed or optional string properties where an empty string should be treated as absent. This decorator only affects the internal deserialization behavior and does not change the public model surface.
