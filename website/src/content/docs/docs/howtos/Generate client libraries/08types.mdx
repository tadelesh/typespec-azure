...
The `@deserializeEmptyStringAsNull` decorator indicates that a string property should be deserialized as `null` when its value is an empty string (`""`). This is useful when working with services that return empty strings to represent absent values.
...
In this example, when the service returns `{"displayName": "", "description": ""}`:

- `displayName` will be deserialized as `null` (because of the decorator)
- `description` will remain as `""` (default behavior)

:::note
This decorator can only be applied to properties of type `string`. Applying it to non-string properties will produce a diagnostic error.
:::
