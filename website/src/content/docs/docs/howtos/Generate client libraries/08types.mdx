...
## Deserializing empty strings as null with `@deserializeEmptyStringAsNull`
...
This is useful when a service returns empty strings to represent absent values, and your client code expects `null` instead.
...
When the service returns `{"displayName": ""}`, the generated client will deserialize the `displayName` property as `null` rather than an empty string.
