...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar derived from `string`) should be deserialized as `null` when its value is an empty string (`""`). This is useful when services return empty strings to represent absent values rather than `null`.
...
@@deserializeEmptyStringAsNull(Demo.Widget.description);
```
...
class Widget(_Model):
    ...
    # During deserialization, empty string "" will be converted to None
```
...
public partial class Widget
{
    ...
    // During deserialization, empty string "" will be converted to null
...
export interface Widget {
  name: string;
  // During deserialization, empty string "" will be converted to undefined
  description?: string;
}
```
...
type Widget struct {
	// REQUIRED
	Name        *string
	// During deserialization, empty string "" will be converted to nil
	Description *string
}
```
