...
When using `DocumentationMode.append`, the client documentation is appended to the existing `@doc` text. This is useful for adding SDK-specific guidance without losing the original API documentation.

```typespec
@doc("Returns the list of active widgets.")
@clientDoc(" Note: Results are cached for 5 minutes.", DocumentationMode.append)
op listWidgets(): Widget[];
```

In the generated SDK, the method documentation will read:
_"Returns the list of active widgets. Note: Results are cached for 5 minutes."_
...
The `@deserializeEmptyStringAsNull` decorator marks a string property so that empty strings (`""`) are deserialized as `null` in the generated SDK. This is useful when a service returns empty strings to represent absent values.

This decorator can only be applied to properties of type `string` or scalar types derived from `string`.
...
class Widget(Model):
    id: str
    name: str
    # When the service returns description: "",
    # the SDK deserializes it as None
    description: Optional[str]
```

```csharp
// TODO: fill in C# example manually — emitter generation failed (missing .NET SDK)
```
...
export interface Widget {
  ...
  name: string;
  // When the service returns description: "",
  // the SDK deserializes it as undefined
  description?: string;
}
```
...
public final class Widget {
    // When the service returns description: "",
    // the SDK deserializes it as null
    private String id;
    private String name;
...
type Widget struct {
    ...
    Name        *string
    // When the service returns description: "",
    // the SDK deserializes it as nil
    Description *string
}
```
...
## Custom JSON Converter (C# Only)

:::caution
The `@useSystemTextJsonConverter` decorator is a C#-specific customization for backward compatibility. It is only relevant for the C# emitter and has no effect on other languages.
:::

The `@Azure.ClientGenerator.Core.useSystemTextJsonConverter` decorator marks a model to use a custom `System.Text.Json` converter in the generated C# SDK. This is used when a model requires specialized JSON serialization/deserialization logic that cannot be expressed through standard TypeSpec patterns.
...
@route("/widgets")
@get
op getWidget(@path id: string): CustomWidget;
```

```python
# @useSystemTextJsonConverter has no effect on Python.
# The model is generated normally:
class CustomWidget:
    id: str
    data: str
```
...
```csharp
// The model is annotated with a custom JsonConverter attribute:
...
```typescript
// @useSystemTextJsonConverter has no effect on TypeScript.
// The model is generated normally:
...
```java
// @useSystemTextJsonConverter has no effect on Java.
// The model is generated normally:
...
```go
// @useSystemTextJsonConverter has no effect on Go.
// The model is generated normally:
