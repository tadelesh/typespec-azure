...
The `@deserializeEmptyStringAsNull` decorator instructs client code generators to treat empty strings (`""`) as `null` during deserialization. This is useful when a service returns empty strings to represent absent values.

This decorator can only be applied to model properties of type `string` or scalar types derived from `string`.
...
// When the `tag` property is an empty string, deserialize it as null
@@deserializeEmptyStringAsNull(PetStore.Pet.tag);
```

```python
class Pet:
    name: str = rest_field()
    # When tag is "", it will be deserialized as None
    tag: str = rest_field()
    age: int = rest_field()
```
...
public partial class Pet
{
    ...
    // When Tag is "", it will be deserialized as null
...
type Pet struct {
    Name *string
    // When Tag is "", it will be deserialized as nil
    Tag  *string
    Age  *int32
}
```
