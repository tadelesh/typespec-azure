...
For brownfield services where an operation is not inherently long-running on the service side but should be treated as an LRO by SDK generators, use the `@markAsLro` decorator from `Azure.ClientGenerator.Core.Legacy`:
...
When applied, TCGC treats the operation as an LRO and SDK generators will generate polling mechanisms, LRO-specific return types, and handle the operation as an asynchronous long-running process. The operation must return a model type.
