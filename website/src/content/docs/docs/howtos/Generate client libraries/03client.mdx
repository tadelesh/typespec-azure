...
If the services being merged declare different versions of a shared library dependency (e.g. `Azure.ResourceManager.CommonTypes`) via `@useDependency`, TCGC will emit a `warning` diagnostic `inconsistent-multiple-service-dependency` listing the conflicting versions. Diverging dependency versions can cause duplicate or incomplete models in the emitted SDK; align the `@useDependency` declarations across services to resolve this warning.
:::
