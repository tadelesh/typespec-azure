...
Two sub clients that separate the operations can be declared using nested `@client` decorators from `typespec-client-generator-core`:

:::note
`@operationGroup` is deprecated. Use `@client` to define sub clients instead.
:::
