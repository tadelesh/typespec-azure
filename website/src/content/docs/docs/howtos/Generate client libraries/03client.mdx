...
It can be used to move operations to an existing sub client defined by `Interface` or `Namespace`, or move to the root client of a namespace with `@service` decorator.
...
### Scoping Operations or Properties to Specific Languages

The `@scope` decorator allows you to control which language emitters generate a specific operation or model property.
By default, all elements are generated for all languages. Use `@scope` to include an element only for specific languages or to exclude it from certain languages using the `!` prefix.
...
@route("/internal")
@get
op goOnlyOperation(): void;

// This operation is generated for all languages except C#
@scope("!csharp")
@route("/skip-csharp")
@get
op skipCsharpOperation(): void;

model Config {
  name: string;

  // This property is only generated for Python and Java
  @scope("python, java")
  debugInfo?: string;
}
```

The supported scope values are: `"python"`, `"csharp"`, `"java"`, `"javascript"`, `"go"`.
You can combine multiple values with commas (e.g., `"python, java"`) or negate with `!` (e.g., `"!csharp"` or `"!(java, python)"`).
