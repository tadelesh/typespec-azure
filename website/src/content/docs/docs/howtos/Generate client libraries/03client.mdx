...
Two sub clients that separate the operations can be declared using the `@client` decorator from `typespec-client-generator-core`. The `@operationGroup` decorator is deprecated and `@client` should be used instead:
