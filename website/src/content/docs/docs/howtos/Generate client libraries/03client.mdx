...
### Omit Client Initialization from Generated Code

There are cases where a client's initialization should be omitted from the generated code and handled manually in custom (hand-written) code. You can signal this by setting `initializedBy` to `InitializedBy.customizeCode`.
...
@@clientInitialization(MySubClient, { initializedBy: InitializedBy.customizeCode });
```

When `InitializedBy.customizeCode` is set, TCGC emitters will omit the constructor or factory method for that client from generated code. The client's initialization must be provided by custom code in the SDK overlay.
