...
Two sub clients that separate the operations can be declared using the `@client` decorator (or the deprecated `@operationGroup` decorator, which is an alias) from `typespec-client-generator-core`:

:::caution
The `@operationGroup` decorator is deprecated. Use `@client` on sub-client interfaces instead. `@operationGroup` still works as an alias for `@client` but will be removed in a future release.
:::
...
# usage
from petstorerenamed import PetStoreClient

client = PetStoreClient(endpoint="<endpoint>")
client.op_grp1.feed()
client.op_grp2.pet()
```
...
PetStoreClient client = new PetStoreClient(new Uri("<endpoint>"));
...
// Usage
PetStoreClientBuilder builder = new PetStoreClientBuilder().endpoint("<endpoint>");
...
func (client *PetStoreClient) NewPetStoreOpGrp1Client() *PetStoreOpGrp1Client
func (client *PetStoreClient) NewPetStoreOpGrp2Client() *PetStoreOpGrp2Client

// generated petstoreopgrp1_client.go
type PetStoreOpGrp1Client struct{}
...
type PetStoreOpGrp2Client struct{}
...
// Usage
petStoreClient, _ := NewPetStoreClientWithNoCredential("<endpoint>", nil)
opGrp1Client := petStoreClient.NewPetStoreOpGrp1Client()
opGrp1Client.Feed(context.Background(), nil)
opGrp2Client := petStoreClient.NewPetStoreOpGrp2Client()
opGrp2Client.Pet(context.Background(), nil)
```
