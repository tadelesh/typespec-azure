...
Two sub clients that separate the operations can be declared using the `@client` decorator from `typespec-client-generator-core`. The `@operationGroup` decorator is an alias for `@client` but is deprecated — prefer `@client` for new specs:
