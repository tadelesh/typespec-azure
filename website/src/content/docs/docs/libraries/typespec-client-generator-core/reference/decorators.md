...
If there is any `@client` definition, then each `@client` is a root client and each nested `@client` is a sub client with hierarchy.
...
:::caution
**Deprecated**: Use `@client` instead. The `@operationGroup` decorator is deprecated and will be removed in a future release. Define sub clients using `@client` directly.
:::
