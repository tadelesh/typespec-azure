...
If there is any `@client` definition, then each root-level `@client` is a root client. Nested `@client` decorators (or the deprecated `@operationGroup`) define sub clients with hierarchy.
