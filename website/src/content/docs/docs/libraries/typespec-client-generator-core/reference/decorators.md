...
If there is any `@client` definition, then each root-level `@client` is a root client and nested `@client` decorators define the sub client hierarchy.
...
:::caution[Deprecated]
`@operationGroup` is deprecated and will be removed in a future release. Use [`@client`](#@Azure.ClientGenerator.Core.client) instead to define sub clients.
:::
