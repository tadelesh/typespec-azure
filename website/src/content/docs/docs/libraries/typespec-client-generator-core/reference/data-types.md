...
| initializedBy? | `EnumMember \| Union` | Determines how the client can be initialized. Use `InitializedBy` enum to set the value. The value can be `InitializedBy.individually`, `InitializedBy.parent`, `InitializedBy.individually \| InitializedBy.parent`, or `InitializedBy.customizeCode` (to omit client initialization from generated code and handle it manually).                                               |
