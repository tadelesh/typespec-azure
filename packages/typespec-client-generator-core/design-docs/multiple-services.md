...
- **`@operationGroup`** _(deprecated)_: Functions as an alias for `@client` — it creates a sub-client (the `SdkOperationGroup` type has been consolidated into `SdkClient`). The same nested `@client` patterns apply. Use `@client` instead; `@operationGroup` will be removed in a future release.
