...
`InitializedBy.individually (1)` (the client could be initialized individually), `InitializedBy.parent | InitializedBy.individually (3)` (both), or `InitializedBy.customizeCode (4)` (initialization omitted from generated code).
...
The above tsp gets client `MyServiceClient` and sub client `InnerGroup`.The `InnerGroup`'s `clientInitialization` model's properties contains a property named `blobName`.
