...
### Azure_ClientGenerator_Core_ResponseAsBool_HeadOperation

- Endpoint: `head /azure/client-generator-core/response-as-bool/exists/{id}`

This scenario tests that a HEAD operation decorated with `@responseAsBool` returns `true`
when the resource exists (2xx response) and `false` when it does not (404 response).

Test case 1 - resource exists:
Expected path parameter: id="exists"
Expected response: 200 OK → method returns `true`

Test case 2 - resource does not exist:
Expected path parameter: id="notExists"
Expected response: 404 Not Found → method returns `false`

### Azure_ClientGenerator_Core_Scope_IncludedForAll

- Endpoint: `get /azure/client-generator-core/scope/included/all`

This scenario tests that an operation with no scope restriction is included for all languages.
Expected query parameter: name="sample"
Expected response body:
...
### Azure_ClientGenerator_Core_Scope_LanguageSpecific

- Endpoints:
  - `get /azure/client-generator-core/scope/excluded/csharpOnly`
  - `get /azure/client-generator-core/scope/excluded/excludeCsharp`

This scenario tests that the `@scope` decorator can be used to include an operation only for
specific languages. An operation scoped to `"csharp"` should only be generated for C#.
The second operation scoped to `"!csharp"` should be generated for all languages except C#.
Expected query parameter: name="sample"
Expected response body:
