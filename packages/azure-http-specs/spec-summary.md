...
### Azure_ClientGenerator_Core_ClientDoc_getAppendDoc

- Endpoint: `get /azure/client-generator-core/client-doc/appendDoc`

Test model with appended documentation using `@clientDoc` with `DocumentationMode.append`.
The generated SDK model should have both the original and client-specific documentation.
Expected response body:
...
### Azure_ClientGenerator_Core_ClientDoc_getReplaceDoc

- Endpoint: `get /azure/client-generator-core/client-doc/replaceDoc`

Test model with replaced documentation using `@clientDoc` with `DocumentationMode.replace`.
The generated SDK model should have the client-specific documentation instead of the original.
Expected response body:
...
Test HEAD operation with @responseAsBool decorator when resource exists.
The HEAD operation should return true for a 200 response.
Expected response status: 200

### Azure_ClientGenerator_Core_ResponseAsBool_notExists

- Endpoint: `head /azure/client-generator-core/response-as-bool/notExists`

Test HEAD operation with @responseAsBool decorator when resource does not exist.
The HEAD operation should return false for a 404 response.
Expected response status: 404

### Azure_ClientGenerator_Core_Scope_allLanguages

- Endpoint: `get /azure/client-generator-core/scope/allLanguages`

Test an operation that is scoped to all languages (no scope decorator).
All language emitters should generate this operation.
Expected query parameter: name="sample"
Expected response body:
...
- Endpoint: `get /azure/client-generator-core/scope/excludeJava`

Test an operation that is scoped to specific languages using `@scope("!java")`.
...
### Azure_ClientGenerator_Core_Scope_scopedProperties

- Endpoint: `put /azure/client-generator-core/scope/scopedProperties`

Test a model with language-scoped properties.
The `pythonProperty` should only be generated for Python.
The `excludeGoProperty` should be generated for all languages except Go.
Expected request body:
